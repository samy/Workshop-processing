...
void draw() {
  background(255);
  fill(couleurs[frameCount % couleurs.length]); //frameCount est le n° de frame
  textSize(map(valeurs[frameCount % valeurs.length], min(valeurs), max(valeurs), 5, 180));
  text(noms[frameCount % noms.length], width/2, height/2);
}
